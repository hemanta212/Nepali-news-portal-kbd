...
from flask_final.forms import SignupForm, LoginForm, RequestResetForm, PasswordResetForm
...
#        if dup == None:
#            news_post = NNN(title  = news['title'], nep_date  = news['date'], source  = news['source'], summary = news['summary'], news_link = news['news_link'],)
...
#        news_list = NNN.query.order_by(NNN.date.desc()).paginate(page=page, per_page=10)
#    
...
def nep_international_news():
    ...
    for news in nagarik_international_news_list[::-1]:
        ...
        if dup == None:        
            news_post =NIN(title  = news['title'], nep_date  = news['date'], source  = news['source'], summary = news['summary'], news_link = news['news_link'])
            db.session.add(news_post)
            db.session.commit()    
    try:
        ...
        page = request.args.get("page",1,type=int)    
        news_list = NIN.query.order_by(NIN.date.desc()).paginate(page=page, per_page=10)
...
def eng_national_news():
    ...
    for news in kathmandu_post_news_list[::-1]:
        ...
        if dup == None:
            news_post = ENN(title  = news['title'], nep_date  = news['date'], source  = news['source'], summary = news['summary'], news_link = news['news_link'],image_link=news['image_link'])
            db.session.add(news_post)
            db.session.commit()
    ...
    try:
        ...
        page = request.args.get("page",1,type=int)    
        news_list = ENN.query.order_by(ENN.date.desc()).paginate(page=page, per_page=10)
...
def send_reset_mail(user):
    ...
    msg = Message('Reset Password for Khabar Board', sender = 'noreply@demo.com', recipients= [user.email])
    msg.body = f'''{user.full_name}, you can have your password reset in below link {url_for('reset_token', token=token, _external =True)} if you have not sent this email then you can just ignore it'''
